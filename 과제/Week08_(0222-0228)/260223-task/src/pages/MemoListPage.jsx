...
export default function MemoListPage() {
  ...
  const handleTogglePin = async (e, memo) => {
    e.preventDefault();
    try {
      await togglePin(memo.id, memo.pinned || memo.isPinned);
      fetchMemos();
    } catch (e) {
      alert(e.message);
    }
  ...
  const isPinned = (memo) => memo.pinned || memo.isPinned;

  const sortedMemos = [
    ...memos.filter((m) => isPinned(m)),
    ...memos.filter((m) => !isPinned(m)),
  ];
  const totalPages = Math.max(1, Math.ceil(sortedMemos.length / PAGE_SIZE));
  const pagedMemos = sortedMemos.slice(
    (page - 1) * PAGE_SIZE,
    page * PAGE_SIZE,
  );
  ...
  return (
    <div className={`${styles.container} page-enter`}>
      ...
      {error && (
        <div className={`${styles.statusMsg} ${styles.errorMsg}`}>{error}</div>
      )}
        ...
        <>
          {pagedMemos.length === 0 ? (
            <div className={styles.emptyState}>
              ...
              {!search && !category && (
                <Link to="/memos/new" className={styles.emptyLink}>
                  첫 메모 작성하기 →
                </Link>
              )}
            </div>
              ...
              {pagedMemos.map((memo, i) => (
                <li
                  ...
                  style={{
                    ...
                    borderColor: isPinned(memo) ? "#ecc94b" : undefined,
                    background: isPinned(memo) ? "#fffaf0" : undefined,
                  }}
                >
                      ...
                      >
                        <button
                          ...
                          style={{
                            ...
                            cursor: "pointer",
                            fontSize: "1rem",
                            opacity: isPinned(memo) ? 1 : 0.3,
                            padding: "0 2px",
                          }}
                          title={isPinned(memo) ? "고정 해제" : "고정"}
                        >
                          📌
                        </button>
